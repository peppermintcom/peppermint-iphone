...
- (instancetype)initWithRow:(NSUInteger)row section:(NSUInteger)section {
  return [self initWithIdentifier:nil section:section row:row];
}
...
- (NSArray *)rowSectionDataForDataSource:(id<IGInterfaceTableDataSource>)dataSource {
  ...
  for (NSInteger section = 0; section < sections; section++) {
    ...
    for (NSInteger row = 0; row < rows; row++) {
      IGTableRowData * indexPath = [[IGTableRowData alloc] initWithRow:row section:section];
...
- (void)configureRowSectionData:(NSArray *)rowSectionData {
  ...
  [rowSectionData enumerateObjectsUsingBlock:^(IGTableRowData *rowData, NSUInteger idx, BOOL *stop) {
    ...
    if (rowData.section != NSNotFound && rowData.row != NSNotFound) {
      [dataSource table:self configureRowController:controller forIndexPath:rowData];
    } else if (rowData.section != NSNotFound) {
    ...
    } else if (rowData.row == NSNotFound
               && [dataSource respondsToSelector:@selector(table:configureSectionController:forSection:)]) {
      [dataSource table:self configureSectionController:controller forSection:rowData.section];
...
- (IGTableRowData *)_indexPathFromRowIndex:(NSInteger)rowIndex {
  IGTableRowData *rowData = [self rowSectionData][rowIndex];
  return rowData;
}

- (IGTableRowData *)indexPathFromRowIndex:(NSInteger)rowIndex {
  IGTableRowData * rowData = [self _indexPathFromRowIndex:rowIndex];
  // for the public method, return nil for section rows
  if (rowData && rowData.row == NSNotFound) {
    return nil;
  }
  return rowData;
}

- (NSInteger)rowIndexFromIndexPath:(IGTableRowData *)indexPath {
...
- (NSInteger)rowIndexFromSection:(NSInteger)section {
  IGTableRowData * indexPath = [[IGTableRowData alloc] initWithRow:NSNotFound section:section];
...
- (void)scrollToSection:(NSInteger)section {
  ...
  if (rowIndex != NSNotFound) {
    ...
    IGTableRowData * rowData = [[IGTableRowData alloc] initWithRow:0 section:section];
    [self scrollToRowAtIndexPath:rowData];
  }
}

- (void)scrollToRowAtIndexPath:(IGTableRowData *)indexPath {
...
- (void)updateRowWithIndexPath:(IGTableRowData *)indexPath {
